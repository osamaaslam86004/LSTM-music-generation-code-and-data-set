...
from music21 import note, chord, stream, key, pitch, meter
import os
import random

# Configuration
DATA_PATH = 'data/notes'
MODEL_PATH = "C:/mmusic250516/weights-200-0.1454.hdf5"  # Your HDF5 model file
...
def load_and_validate_data():
    ...
    print(f"Loaded {len(valid_notes)} valid notes with {len(unique_pitches)} unique pitches")
    return valid_notes, len(unique_pitches), {p:i for i,p in enumerate(unique_pitches)}, {i:p for i,p in enumerate(unique_pitches)}
...
def generate_melody(model, pattern, int_to_pitch, num_notes, key_obj, chords):
    ...
    for i in range(num_notes):
        ...
        constrained_preds = apply_harmonic_constraints(preds, current_chord, int_to_pitch, key_obj)
...
def main():
    ...
    generated_notes = generate_melody(model, initial_pattern, i2p, GENERATE_LENGTH, KEY, CHORD_PROGRESSION)
