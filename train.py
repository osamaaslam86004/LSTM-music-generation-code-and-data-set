...
def train():
    ...
    sequence_length = 100
    network_input, network_output = prepare_sequences(notes, pitch_to_int, sequence_length, num_pitch)
    ...
    print(f"Using {1-validation_split:.0%} for training, {validation_split:.0%} for validation")
    ...
    print(f"Best epoch: {best_epoch} with loss: {history.history['loss'][best_epoch-1]:.4f}")
    print(f"Training accuracy at best epoch: {history.history['accuracy'][best_epoch-1]:.4f}")
    ...
    try:
        ...
        # Learning rate plot
        plt.subplot(2, 1, 2)
        plt.plot(epochs, [float(lr) for lr in history.history['lr']], 'g-', linewidth=2, label='Learning Rate')
